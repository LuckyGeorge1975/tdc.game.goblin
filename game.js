...
function renderUnit(u){
  ...
  if(u.acted&&u.hp>0&&u.team==='player')g.classList.add('acted-unit');if(u.disabled&&u.hp>0){g.classList.add('disabled-unit');g.style.opacity='.32';g.style.filter='grayscale(1)'}
...
function combatOutcome(attacker,target){const strength=Math.max(0,attacker.damage),defense=Math.max(1,target.maxHp),ratio=strength>=defense*5?'5-1':strength*2<defense?'1-2':`${Math.min(4,Math.max(1,Math.floor(strength/defense)))}-1`,table={'1-2':['NE','NE','NE','NE','D','X'],'1-1':['NE','NE','D','D','D','X'],'2-1':['NE','D','D','X','X','X'],'3-1':['D','D','X','X','X','X'],'4-1':['D','X','X','X','X','X']},result=ratio==='5-1'?'X':table[ratio][Math.floor(Math.random()*6)];if(result==='D'){if(target.id==='infantry')target.hp=Math.max(0,target.hp-1);else if(target.disabled)target.hp=0;else {target.disabled=true;target.disabledUntil=turn+(attacker.team==='enemy'?2:1)}}if(result==='X')target.hp=0;return {ratio,result}}function checkVictory(){const core=units.find(u=>u.core),enemies=units.filter(u=>u.team==='enemy'&&u.hp>0);if(!core?.hp&&(currentScenario==='iron-dust'||!enemies.length)){finish(true);return true}if(!units.some(u=>u.team==='player'&&u.hp>0)){finish(false);return true}return false}
function attack(attacker,target){if(attacker.disabled){setToast(`${attacker.name} IST DEAKTIVIERT`);return}attacker.acted=true;const outcome=combatOutcome(attacker,target);target.justHit=true;addLog(`${attacker.name} feuert auf ${target.name}: ${outcome.result} bei ${outcome.ratio}.`);setToast(target.hp?(outcome.result==='D'?'ZIEL DEAKTIVIERT':'KEIN EFFEKT'):'ZIEL AUSGESCHALTET');selected=null;updateSelection();draw();setTimeout(()=>{target.justHit=false;draw()},560);if(checkVictory())return;maybeAutoEndTurn()}function recoverDisabled(team){units.filter(u=>u.team===team&&u.disabled&&u.disabledUntil<=turn).forEach(u=>{u.disabled=false;u.disabledUntil=0;addLog(`${u.name} ist wieder einsatzbereit.`,team==='enemy')})}function enemyTurn(){recoverDisabled('enemy');phase='enemy';$('#phase-title').textContent='HOSTILE PHASE';$('#log-status').textContent='PROCESSING';setToast('FEINDZUG WIRD AUSGEFÜHRT');setTimeout(()=>{const foes=units.filter(u=>u.team==='enemy'&&u.hp>0&&!u.disabled&&u!==units.find(x=>x.id==='core'));foes.forEach(foe=>{const targets=units.filter(u=>u.team==='player'&&u.hp>0).sort((a,b)=>dist(foe,a)-dist(foe,b));const target=targets[0];if(target&&dist(foe,target)<=foe.range){const outcome=combatOutcome(foe,target);target.justHit=true;addLog(`${foe.name} feuert auf ${target.name}: ${outcome.result} bei ${outcome.ratio}.`,true);if(target.hp===0)addLog(`${target.name} ist ausgefallen.`,true);setTimeout(()=>{target.justHit=false;draw()},560)}else if(target){const step=findEnemyStep(foe,target);if(step){foe.moveFrom={x:foe.x,y:foe.y};foe.x=step.x;foe.y=step.y;foe.moveCount=(foe.moveCount||0)+1;addLog(`${foe.name} rückt nach ${String.fromCharCode(65+foe.x)}-${String(foe.y+1).padStart(2,'0')} vor.`,true)}else addLog(`${foe.name} hält Position.`,true)}});if(checkVictory())return;units.filter(u=>u.team==='player').forEach(u=>u.acted=false);phase='player';turn++;recoverDisabled('player');$('#turn-number').textContent=String(turn).padStart(2,'0');$('#phase-title').textContent='PLAYER PHASE';$('#log-status').textContent='STANDBY';setToast('WÄHLE EINE EINHEIT');draw()},650)}
function updateRoster(){const el=$('#unit-roster');el.innerHTML='';units.filter(u=>u.team==='player').forEach(u=>{const card=document.createElement('div');const covered=terrain.has(`${u.x},${u.y}`);card.className=`unit-card ${u.id==='ogre'?'ogre':''} ${u===selected?'active':''} ${u.acted?'acted':''} ${u.hp<=0?'destroyed':''} ${covered&&u.hp>0?'covered':''}`;card.innerHTML=`<div class="unit-token">${u.hp<=0?'×':u.icon}</div><div><span class="unit-name">${u.name}</span><span class="unit-type">${u.hp<=0?'DESTROYED':u.disabled?'DISABLED':covered?'COVERED · '+u.type:u.acted?'DONE / NEXT TURN':u.type}</span></div><div class="hp-bar">${Array.from({length:u.maxHp},(_,i)=>`<i class="${i>=u.hp?'off':''}"></i>`).join('')}</div>`;card.onclick=()=>{if(u.hp>0)selectPlayerUnit(u)};el.appendChild(card)});$('#force-count').textContent=String(units.filter(u=>u.team==='player'&&u.hp>0).length).padStart(2,'0');const core=units.find(u=>u.id==='core');const corePct=Math.round((1-core.hp/core.maxHp)*100);$('#objective-text').textContent=core.hp?`CORE ${core.hp}/${core.maxHp} HP`:'CORE OFFLINE';$('#objective-progress').style.width=`${corePct}%`}
...
function finish(win){gameOver=true;phase='over';$('#modal-title').textContent=win?'MISSION COMPLETE':'MISSION FAILED';$('#modal-copy').textContent=win?(currentScenario==='relay-run'?'Relaisknoten und Eskorte sind ausgeschaltet. Der Korridor ist gesichert.':'Der Kommandokern ist verstummt. Iron Dust gehört wieder euch.'):'Die Verteidigungslinie ist kollabiert. Starte neu und halte den Goblin länger im Zentrum.';$('#modal').classList.remove('hidden');$('#log-status').textContent=win?'SUCCESS':'CRITICAL';}
