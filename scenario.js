function loadScenario(id){
  ...
  units.splice(0,units.length,...JSON.parse(JSON.stringify(config.units)).map(unit=>({...unit,moveCount:0,moveFrom:null,acted:false,justHit:false})));
  ...
  $('#phase-title').textContent='PLAYER PHASE';
